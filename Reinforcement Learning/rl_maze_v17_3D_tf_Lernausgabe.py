...
###################################################################
#### Hyperparameter
num_iterations = 200000                 # Anzahl an Steps
...
move_valid_value = -0.04                        # negativer Reward pro Step
move_illegal = -6                               # negativer Reward pro illegalen Move. Bsp In Mauer laufen
move_to_already_visited_cell_value = -0.25      # negativer Reward für bereits besuchte Felder
move_to_target = 5000                           # positiver Reward für erreichen des Ziels. Abhängig von maze später
min_reward_trigger = -500                       # kleinster Bereich, ab dem Episode beendet wird

proximity_reward_allowed = False                # alternative zu Ziel entfernungs negative reward.  
proximity_reward_ration = 1                     # Faktor wird multipliziert
#avg_distance = np.abs(targetCell[0] - startCell[0]) + np.abs(targetCell[1] - startCell[1]) + np.abs(targetCell[2] - startCell[2])


#### Hyperparamter - Abbruchbedingung
max_steps_per_episode = 70                      # Episode endet nach X steps
max_distance = 5000                             # Distanz einer großen Diagonalen der Maze. abhängig von mazegröße wird später noch nachgeladen 


#### Hyperparamter - Episode Saving
save_episodes = False
list_episodes = []
episode_counter_max = initial_collect_steps + 50
...
#### Hyperparameter - abhängig von geladener Maze
...
class GridWorldEnv(py_environment.PyEnvironment):
    ...
    def _reset(self):
        if(self._episode_counter < episode_counter_max and self._episode_counter > initial_collect_steps):
            list_episodes.append(self._list_episode)
            
        self._list_episode = []
        self._episode_counter += 1
        episodes.append(self._step_iterator)
        episodes.append(self.accumulated_reward)
        self._step_iterator = 0
        self._state=[startCell[0],startCell[1],startCell[2],targetCell[0],targetCell[1],targetCell[2]]
        self.visitedMaze = np.full((dim[0],dim[1],dim[2]),unvisited)
        self.visitedMaze[startCell[0],startCell[1],startCell[2]] = visited
    ...
    def _step(self, action):
        if self._episode_ended or self._step_iterator > max_steps_per_episode or self.accumulated_reward < self.min_reward:
            return self.reset()
        ...
        self._list_episode.append([self._state[0], self._state[1], self._state[2]])
        if self.game_over():
    ...
    def distance_reward(self):
        ...
        reward = -(actual_distance * 0.01) #-(actual_distance * 0.01)
        return reward
...
for i in range(num_iterations):
    ...
    if step % eval_interval == 0:
        ...
        print('step = {0}: Average Return = {1}'.format(step, np.floor(avg_return)))
...
time_record = start_time = datetime.datetime.now() 
final_actions, final_game_status = compute_evaluated_path(eval_env, tf_agent.policy, 1000, final_game_status)
dt = datetime.datetime.now() - start_time
eval_time = dt.total_seconds() * 1000
eval_time = eval_time / 1000
print(str(eval_time) , "millisecond") 

print("final average: ", finale_avg_return)

print(len(episodes) / 2, " episodes in total; ", num_iterations, " number of steps")
episodes_durchschnitt = 0
reward_durchschnit = 0
for i in range(len(episodes)):
    if(i % 2 == 0):
        episodes_durchschnitt += episodes[i]
    else:
        reward_durchschnit += episodes[i]
episodes_durchschnitt = episodes_durchschnitt / (len(episodes) / 2)
reward_durchschnit = reward_durchschnit / (len(episodes) / 2)
print(episodes_durchschnitt, " episodes durchschnittslänge; ", reward_durchschnit, " reward_durchschnit")

def save_episode_files(timestamp, episode_id, final_game_status):
        ...
        coord_file = outdir_path + timestamp + "_" + input_file_name + "_trainings_episode_" + str(episode_id) + ".json"
        
        if (final_game_status == 'win'):
            coord_data = []
            for coord in list_episodes[episode_id]:
                for single_coord in coord:
                    coord_data.append(int(single_coord))
        ...
        if (final_game_status == 'win'):
            ...
            if maze_file_path is not None:
                with open(maze_file_path, "r") as import_file, open(coord_file, "w") as export_file:
                    ...
                    export_data = dict(import_data)
                    export_data['resultPath'] = coord_data
                    export_data['resultTime'] = eval_time
                    json.dump(export_data, export_file, indent=3)
            # If no file imported, just dump the resultPath; for testing only
            else:
                with open(coord_file, "w") as export_file:
